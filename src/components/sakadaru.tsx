...
function line(
  ...
  if (length < center.length) {
    ...
    const sideLength = (length - center.length) / 2;
    return (
      decoration.repeat(sideLength).substr(0, sideLength) +
      center +
      decoration.repeat(sideLength).substr(0, sideLength)
    );
  }
...
function spacer(
  ...
  return center.map(
    (value) =>
      decoration.repeat(spaces).substr(0, spaces) + value,
  );
}
...
function side(content: Array<string>): Array<string> {
  ...
  return content.map(function (line) {
    rownum++;
    width =
      rownum > height / 2 ? height - rownum + 1 : rownum;
    return (
      ' '.repeat(height - width) +
      '('.repeat(width) +
      line +
...
const Sakadaru = (props: SakadaruPropertyType) => {
  const sakadaruDeoration = (data: string) => {
    ...
    letters = letters.map(function (line) {
      length =
        line.length > length ? line.length + 1 : length;
      return line + ' ';
    });
    // 酒樽の装飾をつける
    let output = spacer(header(length), height);
