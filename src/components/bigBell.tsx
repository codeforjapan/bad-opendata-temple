...
`;
const BigBellImg = styled((props) => <GatsbyImage {...props} />)`
  flex: 1 1 auto;
  align-items: flex-end;
`;

const StickImg = styled((props) => <GatsbyImage {...props} />)`
  pointer-events: none;
  display: ${(props) => (props.display ? 'flex' : 'none')};
`;
...
const BigBell = () => {
  ...
  stick1: file(relativePath: {eq: "kanetukibou_1.png"}) {
    childImageSharp {
      gatsbyImageData(layout: FULL_WIDTH)
    }
  }
  stick2: file(relativePath: {eq: "kanetukibou_2.png"}) {
    childImageSharp {
      gatsbyImageData(layout: FULL_WIDTH)
    }
  }
...
`,
  ...
  return <>
      ...
      <StickContainer>
        ...
        >
          <StickImg
            ref={stick1Img}
            display={!isWoundUp}
            fluid={data.stick1.childImageSharp.gatsbyImageData}
            alt="鐘付き棒"
          />
          <StickImg
            ref={stick2Img}
            display={isWoundUp}
            fluid={data.stick2.childImageSharp.gatsbyImageData}
            alt="振りかぶった鐘付き棒"
          />
        </Rnd>
        <audio ref={el}>
    ...
    <BigBellContainer display>
      <div>
        <BigBellImg
          fluid={data.bigBell.childImageSharp.gatsbyImageData}
          alt="除夜の鐘"
        />
      </div>
    </BigBellContainer>
